<template>
  <AppLayout>
    <div class="mx-auto flex max-w-md flex-col items-center space-y-6 py-8">
      <div class="flex items-center gap-2 animate-slide-up [animation-fill-mode:backwards]" style="animation-delay: 0ms">
      ...
      <div v-if="qrUrl" class="rounded-2xl bg-white p-6 shadow-lg ring-1 ring-gray-100 dark:bg-dark-800 dark:ring-dark-700 animate-slide-up [animation-fill-mode:backwards]" style="animation-delay: 50ms">
        <canvas ref="qrCanvas" class="mx-auto"></canvas>
      </div>
      <!-- Scan prompt for QR code -->
      <p v-if="qrUrl && !expired && scanHint" class="text-center text-sm text-gray-500 dark:text-gray-400 animate-slide-up [animation-fill-mode:backwards]" style="animation-delay: 100ms">
        {{ scanHint }}
      </p>
      <div v-if="expired" class="text-center animate-slide-up [animation-fill-mode:backwards]" style="animation-delay: 150ms">
      ...
      <div v-else class="text-center animate-slide-up [animation-fill-mode:backwards]" style="animation-delay: 150ms">
      ...
      <a v-if="payUrl && !qrUrl && !expired" :href="payUrl" target="_blank" rel="noopener noreferrer"
        class="btn btn-primary w-full py-3 animate-slide-up [animation-fill-mode:backwards]" style="animation-delay: 200ms">
        {{ t('payment.qr.openPayWindow') }}
      </a>
      <!-- Cancel button -->
      <button v-if="!expired && orderId" class="btn btn-secondary w-full animate-slide-up [animation-fill-mode:backwards]" style="animation-delay: 250ms" :disabled="cancelling" @click="handleCancel">

        ...
        <template #cell-latency="{ row }">
          <div class="flex items-stretch gap-2">
            ...
            <div class="grid grid-cols-[max-content_max-content] items-baseline gap-x-2 gap-y-0.5 text-xs">
              ...
              <template v-if="formatTps(row)">
                <span class="text-gray-400 dark:text-gray-500">t/s</span>
                <span class="font-medium tabular-nums text-cyan-600 dark:text-cyan-400">{{ formatTps(row) }}</span>
              </template>
            </div>
          </div>
        </template>

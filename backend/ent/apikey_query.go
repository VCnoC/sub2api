...
func (_q *APIKeyQuery) QueryGroupBindings() *APIKeyGroupQuery {
...
func (_q *APIKeyQuery) WithGroupBindings(opts ...func(*APIKeyGroupQuery)) *APIKeyQuery {
	query := (&APIKeyGroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withGroupBindings; query != nil {
		if err := _q.loadGroupBindings(ctx, query, nodes,
			func(n *APIKey) { n.Edges.GroupBindings = []*APIKeyGroup{} },
			func(n *APIKey, e *APIKeyGroup) { n.Edges.GroupBindings = append(n.Edges.GroupBindings, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *APIKeyQuery) loadGroupBindings(ctx context.Context, query *APIKeyGroupQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *APIKeyGroup)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)

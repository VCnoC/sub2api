...
type GroupQuery struct {
	...
	withAPIKeyBindings    *APIKeyGroupQuery
...
// QueryAPIKeyBindings chains the current query on the "api_key_bindings" edge.
func (_q *GroupQuery) QueryAPIKeyBindings() *APIKeyGroupQuery {
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withAPIKeyBindings:    _q.withAPIKeyBindings.Clone(),
...
// WithAPIKeyBindings tells the query-builder to eager-load the nodes that are connected to
// the "api_key_bindings" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithAPIKeyBindings(opts ...func(*APIKeyGroupQuery)) *GroupQuery {
	query := (&APIKeyGroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAPIKeyBindings = query
	return _q
}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withAPIKeyBindings; query != nil {
		if err := _q.loadAPIKeyBindings(ctx, query, nodes,
			func(n *Group) { n.Edges.APIKeyBindings = []*APIKeyGroup{} },
			func(n *Group, e *APIKeyGroup) { n.Edges.APIKeyBindings = append(n.Edges.APIKeyBindings, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadAPIKeyBindings(ctx context.Context, query *APIKeyGroupQuery, nodes []*Group, init func(*Group), assign func(*Group, *APIKeyGroup)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.APIKeyGroup(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.APIKeyBindingsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err

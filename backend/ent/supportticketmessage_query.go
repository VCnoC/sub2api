...
func (_q *SupportTicketMessageQuery) QueryAuthor() *UserQuery {
...
func (_q *SupportTicketMessageQuery) WithAuthor(opts ...func(*UserQuery)) *SupportTicketMessageQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SupportTicketMessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*SupportTicketMessage, error) {
	...
	if query := _q.withAuthor; query != nil {
		if err := _q.loadAuthor(ctx, query, nodes, nil,
			func(n *SupportTicketMessage, e *User) { n.Edges.Author = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *SupportTicketMessageQuery) loadAuthor(ctx context.Context, query *UserQuery, nodes []*SupportTicketMessage, init func(*SupportTicketMessage), assign func(*SupportTicketMessage, *User)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*SupportTicketMessage)
	for i := range nodes {
...
func (_q *SupportTicketMessageQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withAuthor != nil {
			_spec.Node.AddColumnOnce(supportticketmessage.FieldAuthorID)
		}
	}

...
func TestMultiSubscriptionMigrationsDefineIndependentEntitlements(t *testing.T) {
	linkMigration, err := FS.ReadFile("183_add_payment_order_subscription_link.sql")
	...
	indexMigration, err := FS.ReadFile("184_multi_subscription_candidate_indexes_notx.sql")
	require.NoError(t, err)
